...
Route.prototype.drawRoute = function (ctx) {
    ...
    if (self.options.orientation === 'horizontal') {
        var from_x_hori =
            self.options.width * self.options.scaleFactor -
            (self.commit.idx + 0.5) *
                self.options.x_step *
                self.options.scaleFactor;
        var from_y_hori =
            (self.from + 1) * self.options.y_step * self.options.scaleFactor;

        var to_x_hori =
            self.options.width * self.options.scaleFactor -
            (self.commit.idx + 0.5 + 1) *
                self.options.x_step *
                self.options.scaleFactor;
        var to_y_hori =
            (self.to + 1) * self.options.y_step * self.options.scaleFactor;
        ...
        if (from_y_hori === to_y_hori) {
            ...
            ctx.bezierCurveTo(
                from_x_hori -
                    ((self.options.x_step * self.options.scaleFactor) / 3) * 2,
                from_y_hori +
                    (self.options.y_step * self.options.scaleFactor) / 4,
                to_x_hori +
                    ((self.options.x_step * self.options.scaleFactor) / 3) * 2,
                to_y_hori -
                    (self.options.y_step * self.options.scaleFactor) / 4,
        ...
        var from_x =
            self.options.width * self.options.scaleFactor -
            (self.from + 1) * self.options.x_step * self.options.scaleFactor;
        var from_y =
            (self.commit.idx + 0.5) *
            self.options.y_step *
            self.options.scaleFactor;
        ...
        var to_x =
            self.options.width * self.options.scaleFactor -
            (self.to + 1) * self.options.x_step * self.options.scaleFactor;
        var to_y =
            (self.commit.idx + 0.5 + 1) *
            self.options.y_step *
            self.options.scaleFactor;
        ...
        if (from_x === to_x) {
            ...
            ctx.bezierCurveTo(
                ...
                from_y +
                    ((self.options.y_step * self.options.scaleFactor) / 3) * 2,
...
Commit.prototype.drawDot = function (ctx) {
    ...
    if (self.options.orientation === 'horizontal') {
        var x_hori =
            self.options.width * self.options.scaleFactor -
            (self.idx + 0.5) * self.options.x_step * self.options.scaleFactor;
        var y_hori =
            (self.dot_offset + 1) *
            self.options.y_step *
            self.options.scaleFactor;
        ...
        ctx.arc(
            x_hori,
            y_hori,
            radius * self.options.scaleFactor,
            0,
            2 * Math.PI,
            true
        );
        ...
        var x =
            ...
            (self.dot_offset + 1) *
                ...
                self.options.scaleFactor;
        var y =
            (self.idx + 0.5) * self.options.y_step * self.options.scaleFactor;
        ctx.fillStyle = self.graph.get_color(self.dot_branch);
        ctx.beginPath();
