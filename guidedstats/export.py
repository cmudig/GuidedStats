...
def exportReport(fittedModel):
    ...
    reportAvailable = hasattr(fittedModel, 'df_resid') and hasattr(fittedModel, 'df_model') and hasattr(fittedModel, 'fvalue') and hasattr(fittedModel, 'f_pvalue') and hasattr(fittedModel, 'rsquared') and hasattr(fittedModel, 'params') and hasattr(fittedModel, 'conf_int') and hasattr(fittedModel, 'pvalues') and hasattr(fittedModel, 'bse')
    
    if not reportAvailable:
        raise ValueError("The fitted model does not have all the necessary attributes for reports")
    else:
